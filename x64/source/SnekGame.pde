...
class Snek {
  ...
  // update function
  void update(Food[] environment, boolean playing) {
    if (playing) {
      ...
      if (control == 1) {
        this.direction -= (1.0/16.0) * QUARTER_PI;
      } else if (control == 2) {
        this.direction += (1.0/16.0) * QUARTER_PI;
      }
...
void draw() {
  ...
  fill(36, 33, 36, constrain((3000.0 - millis())*(51.0)/(100), 0, 255));
  rect(0, 0, width, height);
    
  // Snek v0.0, credits & controls
  fill(255, 255, 255, constrain((3000.0 - millis())*(51.0)/(100), 0, 255));
  textAlign(CENTER);
